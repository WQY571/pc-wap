...
window.tagcloud = (function(win, doc) { // ns
    ...
    function TagCloud (options) {
        ...
        TagCloud._on(self.keep ? win : self.box, 'mousemove', function (ev) {
            ...
            self.mouseX = (oEvent.clientX - (boxPosition.left + self.box.offsetWidth / 3)) / 5;
            self.mouseY = (oEvent.clientY - (boxPosition.top + self.box.offsetHeight / 3)) / 5;
        });
        ...
        self.box.style.minHeight = 0.5 * self.size + "px";
        self.box.style.minWidth = 0.7 * self.size + "px";
        for (var j = 0, len = self.items.length; j < len; j++) {
    ...
    TagCloud.prototype = {
        ...
        update: function () {
            ...
            for (var j = 0, len = self.items.length; j < len; j++) {
                ...
                if(self.index==j){
                    ...
                    self.items[j].fontsize = Math.ceil(per * 2) + self.fontsize - 20;
                    self.items[j].alpha = 1.5 * per - 0.5;
                    self.items[j].element.style.zIndex = Math.ceil(per*10-5);
                }
                self.items[j].element.style.fontSize = self.items[j].fontsize + "px";
                self.items[j].element.style.left = self.items[j].x + (self.box.offsetWidth - self.items[j].offsetWidth) / 3 + "px";
                self.items[j].element.style.top = self.items[j].y + (self.box.offsetHeight - self.items[j].offsetHeight) / 3 + "px";
        ...
        _getItems: function () {
            ...
            for (var i = 0; i < length; i++) {
                ...
                item.element.style.left = item.x + (self.box.offsetWidth - item.offsetWidth) / 3 + "px";
                item.element.style.top = item.y + (self.box.offsetHeight - item.offsetHeight) / 3 + "px";
                items.push(item);
            }
